...
# --- Fix 2b: embed compiler settings (including meshdir) directly inside the
# URDF via MuJoCo's <mujoco> extension tag. Setting spec.compiler.meshdir
# *after* from_file() was too late -- mesh resolution appears to happen
# during parsing itself, before our Python-side settings took effect (that's
# why every mesh was silently dropped despite everything else being correct).
mujoco_tag = (
    f'<mujoco><compiler meshdir="{MESH_DIR}" balanceinertia="true" '
    f'boundmass="0.001" discardvisual="false"/></mujoco>'
)
urdf_text = urdf_text.replace('<robot name="robot">',
                               '<robot name="robot">\n' + mujoco_tag, 1)

# Write the patched URDF out to a real file -- loading via from_file() gives
# MuJoCo normal filesystem access for resolving the mesh assets.
PATCHED_URDF_PATH = os.path.join(BASE_DIR, "urdf", "robot_patched.urdf")
with open(PATCHED_URDF_PATH, "w") as f:
    f.write(urdf_text)

spec = mujoco.MjSpec.from_file(PATCHED_URDF_PATH)

# These are now redundant with the embedded <mujoco> tag above, but harmless
# to set again for clarity / in case any of them weren't picked up.
